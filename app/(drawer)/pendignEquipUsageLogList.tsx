...
const PendingEquipUsageLogList = () => {
  ...
  useEffect(() => {
    if (equipmentLogs.length > 0) {
      const pendingLogs = equipmentLogs.filter(
        (log) =>
          log["Maintenance Completed"] === false &&
          log["Last Maintenance"] === false
      );

      setFilteredLogs(pendingLogs);
    }
    ...
    return (
      <View style={styles.container}>
        <View style={styles.headerContainer}>
          <Text style={styles.title}>Pending Equipment Usage Log List</Text>
        </View>
...
export default PendingEquipUsageLogList;
